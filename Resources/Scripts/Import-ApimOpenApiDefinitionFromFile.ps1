...
try {
    ...
    if($null -eq $versionSetLookup)
    {
        ...
        $versionSet = New-AzApiManagementApiVersionSet -Context $context -Name "$ApiTitle" -Scheme Header -HeaderName "version" -Description "$ApiTitle" -ErrorAction Stop -Verbose:$VerbosePreference
