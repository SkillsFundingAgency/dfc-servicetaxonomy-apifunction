...
namespace DFC.ServiceTaxonomy.ApiFunction.Helpers
{
    ...
    public class Neo4JHelper : INeo4JHelper, IDisposable
    {
        ...
        public Neo4JHelper(IOptionsMonitor<ServiceTaxonomyApiSettings> serviceTaxonomyApiSettings, ILogger log)
        {
            ...
            {
                log.LogWarning( "No credentials for Neo4j database in settings, attempting connection without authorization token.",null);
            }
            else
            {
        ...
        public async Task<object> ExecuteCypherQueryInNeo4JAsync(string query, IDictionary<string, object> statementParameters, ILogger log)
        {
            if (_neo4JDriver == null)
            {
                _log4j = new DriverLogger(log);
                log.LogInformation("Making initial bolt connection");
                _neo4JDriver = GraphDatabase.Driver(_neo4JUrl, _authToken, o => o.WithLogger(_log4j));
            }
